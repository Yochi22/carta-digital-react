...
export function TableDetailsAdmin() {
  ...
  useEffect(() => {getOrdersByTable(id, "", "ordering=-status,created_at")}, [id, reloadOrders]);

  useEffect(() => {getTable(id)}, [id]);

  useEffect(() => {(async () => {const response = await getPaymentByTable(id); if (size(response) > 0) setPaymentData(response[0]);})();}, [reloadOrders]);
